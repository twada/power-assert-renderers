...
SuccinctRenderer.prototype.init = function (traversal) {
    ...
    traversal.on('esnode', function (esNode) {
        ...
        if (!esNode.getParent() && esNode.currentNode.type === 'Identifier') {
            _this.events.push({value: esNode.value(), loc: esNode.location()});
        }
        if (!esNode.getParent() && esNode.currentNode.type === 'MemberExpression') {
            _this.events.push({value: esNode.value(), loc: esNode.location()});
        }
        if (!esNode.getParent() && esNode.currentNode.type === 'CallExpression') {
            _this.events.push({value: esNode.value(), loc: esNode.location()});
        }
